...
ComprobarLoginPanel();
$strHtmlFrame = CabeceraPanelControl(SEC_ADM_ROLES);
echo $strHtmlFrame;
...
if($bBorrar){
	...
	{
		$PA = "DELETE FROM tbUsuarioRol WHERE UsuarioRol_ID = ".intval($ID);
		//echo $PA;
		//die(); 
		ExecPA($PA);
		$strMsg .= TextoDeIdioma("Borrando") . " " . TextoDeIdioma("rol") . "... #" . $ID . ".<br />";
  ...
  if ($borrados > 0)
	{
		if ($borrados == 1)
		{
			$strMsg .= "<br />" . TextoDeIdioma("Se_ha_borrado") . " " . $borrados . " " . TextoDeIdioma("rol") . ".<br />";
		}else{
			$strMsg .= "<br />" . TextoDeIdioma("Se_han_borrado") . " " . $borrados . " " . TextoDeIdioma("roles") . ".<br />";
		}
	}
}
...
if ($bBuscar){
	...
	if($activo >= 0) {
		$strSQLWhere.= " AND (UR.bActivo = ".intval($activo).") ";
	...
	if($texto != "")
	{
		...
		while($tok)
		{
			$subWHERE[0][$i] = "UPPER(UR.Nombre) LIKE '%".addslashes(strtoupper($tok))."%'";
			$tok = strtok(" ");
			$i++;
		}
...
$strSQLCount = "SELECT COUNT(*) FROM tbUsuarioRol UR".$strSQLWhere;
...
?>
	...
	<div class="page-header">
		...
		<div class="pull-right">
	        ...
	        <li><?php echo TextoDeIdioma("Roles")?><span class="divider">/</span></li>
				<li class="active"><?php echo TextoDeIdioma("Listado_de_roles")?></li>
			</ul>
		</div>
	</div>
	...
	<?php
	$PA = "SELECT DISTINCT ";
	$PA .= "		UR.UsuarioRol_ID, UR.Nombre";
	$PA .= "		, UR.bActivo";
	$PA .= " FROM 	tbUsuarioRol UR";
	$PA .= $strSQLWhere;
	$PA .= $strSQLOrden;
			...
			<div class="span12">
				<div class="box">
					...
					<div class="box-head">
						...
						<span><?php echo TextoDeIdioma("Roles")?></span>
					</div>
					<div class="box-body box-body-nopadding">
						<div class="alert alert-info">
						...
						<form action="index.php" name="frmBorrar" method="post">
							...
							<table class="table table-nomargin table-bordered table-striped table-hover table-pagination">
								<thead>
									<tr>
										<?php echo CampoCabeceraTablaListado("UsuarioRol_ID", TextoDeIdioma("Codigo"), $orden, $paginaHtml, $paramsFiltro, $htmlTipoOrdenASC, $htmlTipoOrdenDESC, "center");?>
								...
								<?php 
									...
									for($i=0;$i<$NumElementos;$i++)
									{
										...
										$strHtml .= "<tr>";
										$strHtml .= "	<td style=\"text-align:center;vertical-align:middle\">".$UsuarioRol_ID."</td>";
										$strHtml .= "	<td style=\"vertical-align:middle\">".$Nombre."</td>";
										$strHtml .= "	<td style=\"text-align:center;vertical-align:middle\">";
										if ($esActivo == 1)
										{
											//$strHtml .= "		<img src=\"".RUTA_ADM."img/icons/check.png\" alt=\"".TextoDeIdioma("Activo")."\" />";
											$strHtml .= "		<i class=\"icon-ok\"></i>";
										}else{
											//$strHtml .= "		<img src=\"".RUTA_ADM."img/icons/busy.png\" alt=\"".TextoDeIdioma("No_activo")."\" />";
											$strHtml .= "		<i class=\"icon-remove\"></i>";
										}
										$strHtml .= "</td>";
										$strHtml .= "	<td style=\"text-align:center;vertical-align:middle\"><a href=\"editar.php?UsuarioRol_ID=".$UsuarioRol_ID."\"><img src=\"".RUTA_ADM."/img/icons/attibutes.png\" alt=\"".TextoDeIdioma("Editar")."\" width=\"32\" /></a></td>";
										$strHtml .= " <td style=\"text-align:center;vertical-align:middle\"><input type=\"checkbox\" name=\"chkBorrar[]\" value=\"".$UsuarioRol_ID."\" /></td>";
										$strHtml .= "</tr>";
									}
									echo $strHtml;
									?>
